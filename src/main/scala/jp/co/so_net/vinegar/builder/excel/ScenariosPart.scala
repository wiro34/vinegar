...
trait ScenariosPart extends HeightCalculator {
  self: ExcelGenerator =>
  ...
  private[this] case class StepEnv(scenarioId: Int, stepId: Int) {
    ...
    suite.scenarios.zipWithIndex
      .foldLeft(columnedSheet(sheet, suite)) {
        case (s, (scenario, index)) =>
          ...
          givenSteps(scenarioHeader(s, scenario, env.scenarioId), scenario.groups)(env)
      }
  }
  ...
  private def givenSteps(sheet: Sheet, givenGroups: Seq[GivenGroup])(env: StepEnv): Sheet = {
    givenGroups match {
      case head :: tail =>
        val givenText = head.steps.map(_.mkString).mkString(newLine * 2)
