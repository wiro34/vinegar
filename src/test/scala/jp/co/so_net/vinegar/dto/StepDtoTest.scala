...
//        pprint.pprintln(givens)
        givens should ===(Seq(
          Given(Some("ホームディレクトリに移動する"), None, Seq(
            When(Some("\"vinegar -o ~/Desktop /tmp/vinegar/example.feature\" コマンドを実行する"), None, Seq(
          ...
          Given(Some("ホームディレクトリに移動する"), None, Seq(
            When(Some("\"vinegar /tmp/vinegar/example.feature --out /tmp/vinegar/path/to/deep/dir\" コマンドを実行する"),
            ...
            When(Some("\"vinegar /tmp/vinegar/example.feature --out /tmp/vinegar/path/to/deep/dir -f\" コマンドを実行する"), None, Seq(
