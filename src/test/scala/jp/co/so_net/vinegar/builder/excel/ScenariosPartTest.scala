...
class ScenariosPartTest extends FlatSpec with Matchers with FixtureLoader {
  ...
  it should "add " in {
    sheet.rows(2).cells.toSeq(0).value shouldBe "id"
    sheet.rows(2).cells.toSeq(1).value shouldBe "クッキーを削除する\n# コメントサンプルです。\n\nログインページを開く"
    sheet.rows(2).cells.toSeq(2).value shouldBe "IDとパスワードを入力する\n# ID: test\n# PW: test1234\n\nログインボタンをクリックする"
    sheet.rows(2).cells.toSeq(3).value shouldBe "トップページに遷移すること"

    sheet.rows(3).cells.toSeq(0).value shouldBe "id"
    ...
    sheet.rows(4).cells.toSeq(0).value shouldBe "id"
    ...
    sheet.rows(5).cells.toSeq(0).value shouldBe "id"
    ...
    sheet.rows(6).cells.toSeq(0).value shouldBe "id"
